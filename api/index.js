...
bot.start(async (ctx) => {
  const keyboard = Markup.inlineKeyboard([
    [Markup.button.webApp('🎵 VKMusicX Premium', `https://${process.env.VERCEL_URL || ctx.botInfo.username}.vercel.app`)],
    [{ text: '💰 Kunlik bonus', callback_ 'daily_reward' }],
    [{ text: '👥 Do'stlarim', callback_ 'referrals' }],
    [{ text: '⭐ VIP sotib olish', callback_ 'buy_vip' }]
  ]);
...
Mini App 👇
  `, {
    parse_mode: 'HTML',
    reply_markup: {
      inline_keyboard: [[Markup.button.webApp('🎵 VKMusicX Premium', `https://${process.env.VERCEL_URL || ctx.botInfo.username}.vercel.app`)]]
    }
  });
});
